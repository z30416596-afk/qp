...
export default function SequenceFlowchart({ setName }) {
  ...
  const generateStory = async () => {
    ...
    try {
      ...
      const systemPrompt = `You are a Quran memorization coach. When given first words of ayahs in sequence, create a SHORT mnemonic story in English (under 80 words) that uses those Arabic words as anchors to help remember the ayahs in order. The story should be memorable and creative. Only return the story text, nothing else.`;
      ...
      const res = await fetch(`${API_BASE}/coach/chat`, {
        ...
        body: JSON.stringify({
          system: systemPrompt,
          messages: [{ role: "user", content: `Create a mnemonic story for this sequence of first words: ${firstWords}` }],
        }),
      });
      ...
      const json = await res.json();
      const generatedText = json.content?.[0]?.text || json.message || "Could not generate story.";
